...
- **P-5**（起票 2026-07-12、対象: バックログ B-15 / 0a83add）: プロトタイプ「ものがたりダイス」（`drafts/monogatari-dice/`）を実機で触ってみて、apps/ への移植（＝公開。Tier D ゲート）に進めますか？
  - 推奨: 保留（次回メンター実機セッションで他ツールと一緒に確認してから判断）。根拠: ①drafts 制度の趣旨どおり実物を見てからの判断が安全 ②公開前のため急ぐ理由がない ③28日判断が付かない場合は制度により削除候補として凍結
  - 承認すると: 移植・ポータル掲載・updates.json 追記を1つの人間確認ゲートとして実施 ／ 却下すると: drafts から削除 ／ 可逆性: 移植前ならいつでも削除・修正可能
  - 回答:

- **P-6**（起票 2026-07-12、対象: バックログ C-5 / 11f2ab9）: C-5 監査の修正候補1〜4（終了後30秒削除タイマーの競合修正6アプリ・ito の cleanupTimer 扱い・iisen-show の退出片付け・word-wolf の消滅時復帰）を Tier B 項目として昇格しますか？
  - 推奨: 候補1のみ承認（正常終了パスで確実に起こる孤児化で、修正は「タイマーID保持＋goToTop 時に即時削除」の機械的な型。1アプリ×1コミットでAI動作確認つき）。候補2〜4は影響が小さく保留可
  - 承認すると: 承認された候補を B-16 以降として起票しループが消化 ／ 却下すると: 報告書の記録のみで終了 ／ 可逆性: いつでも再起票可能
  - 回答:

- **P-7**(起票 2026-07-12、対象: バックログ C-9 / a117b3c): C-9 の統一案（①未保護11アプリの作成/参加ボタンに try/catch＋子ども向けエラー文言＋復帰、②name-change の initFirebase 移行=B-8残件）を Tier B 項目として昇格しますか？
  - 推奨: ①②とも承認（①は失敗時に子どもへ何も伝わらない現状の改善で apps/shared に触れない・1アプリ×1コミット、②は挙動同等の機械置換に近い）。nitaku-board は見送りで可
  - 承認すると: B-17（①）・B-18（②）として起票しループが消化 ／ 却下すると: 報告書の記録のみで終了 ／ 可逆性: いつでも再起票可能
  - 回答:
