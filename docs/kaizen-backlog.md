...
- **P-14**（起票 2026-09-05、対象: C-10 / P2-1。実測報告: `docs/reports/c10-emulator-results-2026-09-05.md`）: do-mannaka のホスト自動再接続後も `hostConnected:false` が残る問題を、Tier B の個別修正として進めてよいか。
  - 推奨: 承認。ブラウザ層の切断・自動再接続後にも false が残り、TTL 経過後に試験ルームが消失した。まず実測した1アプリだけを対象とし、他アプリへの一括適用はしない。
  - 受入条件: 存在・状態を確認して接続フラグを復元し、onDisconnect を再登録。削除済み room の再生成や退出後のリスナー残存を防ぐ。host/guest・10秒切断復帰・TTL・明示退出を Emulator で確認。apps/shared・rules は変更しない。name-change のゲーム中TTLは30分で、別途確認後に別単位で扱う。
  - 承認すると: do-mannaka 1アプリ×1関心事の Tier B を採番し実装・独立レビュー ／ 却下・保留すると: 修正せず現状維持 ／ 可逆性: アプリ単位で revert 可。P-11/P-12 のゲスト復帰方式とは別判断。
  - 回答:
