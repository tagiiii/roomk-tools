...
- **P-11**（起票 2026-09-04、対象: 監査 P4-1 / P2-2、報告書 `docs/reports/audit-run-2026-09-04-fable-5-1.md`）: ゲーム中のゲストがリロードまたは一時切断したとき、共通規約「ゲスト切断＝`onDisconnect().remove()`」（AGENTS.md「切断時の挙動」）に例外を足して復帰できるようにしてよいか。静的監査では、旧接続の remove が復元読み取りより先に完了した場合、magire-eshi / ikutsu-ieru / name-change / kaburazu-hint / word-wolf / tatoe-narabe で復帰・再参加を拒否し得る経路を確認した（C-10 の試験ルームでは word-wolf のウルフゲスト消失後に citizenWin:false、tatoe-narabe は試験中の復帰・再参加を拒否。ikutsu-ieru は自 AGENTS.md:254-257 の期待と矛盾）。
  - 実測追記（2026-09-05 C-10）: `docs/reports/c10-emulator-results-2026-09-05.md` を参照。word-wolf はブラウザ層の自動再接続後も player 未復元、次操作で ready のみの部分ノードとなる。iOS Safari はオーナー指示により今回未実施。方式は未承認のまま。
  ...
  - 推奨: C-10 の限定結果を踏まえ案③を検討する。理由: 子どもの誤タップ1回でゲームが壊れるのは体験上の実害が大きく、案②では word-wolf / tatoe-narabe の秘密情報が戻らない。影響: 切断表示と TTL 掃除の挙動が変わるため、採用アプリごとに AGENTS.md へ明記が必要。関連: P2-1 / P2-2（同じ関数に同居させると1経路で閉じる）、B-21 follow-up
