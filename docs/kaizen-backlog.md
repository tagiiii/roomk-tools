...
- **P-1**（起票 2026-07-11、対象: バックログ D-2）: firestore.rules の CI デプロイ化に必要な GCP 権限付与（roles/serviceusage.serviceUsageConsumer）を、あなた自身の作業として予定に入れますか？
  ...
  - 回答:
- **P-2**（起票 2026-07-11、対象: `docs/reports/c1-nandoku-verification-batch1.md` の軽微メモ4件）: nd10 麻績村などの「修正必須ではない」4件は現状維持でよいですか？
  - 推奨: 承認（現状維持）。根拠: ①報告書自身が「子ども向けとして許容」と判定 ②修正すると C-1 の残り7回でも同基準の軽微修正が雪だるま式に増える ③誤りではなく表現の精密さの問題
  - 承認すると: 4件はクローズ、以後の C-1 でも同レベルは起票しない ／ 却下すると: 4件を B 項目として起票 ／ 可逆性: いつでも方針変更可能
  - 回答:
- **P-3**（起票 2026-07-11、対象: `docs/reports/c4-duplicate-phrases.md` 項目4）: kotoba-asobo の u1s02 と u5s33 のクールダウン質問完全一致（「いちばん気に入ったことばを教えて」）は仕様として許容しますか？
  - 推奨: 承認（仕様）。根拠: ①レポート自身が「定番質問として自然」と判定 ②u1（序盤）と u5（終盤）で年間プログラム内の距離が十分 ③クールダウンは毎回変える必要のない儀式的パート
  - 承認すると: クローズ ／ 却下すると: どちらかの差し替えを B 項目として起票 ／ 可逆性: あり
  - 回答:
- **P-4**（起票 2026-07-11、対象: B-1〜B-13 の「体験レビュー待ち」）: 累積した体験レビュー（howto 文言・アイコン変更・jinro 配色など13項目分）を、次回の実機セッション時にまとめて確認する方式でよいですか？
  - 推奨: 承認（まとめて事後確認。再確認日 2026-07-25）。根拠: ①ガードレール上、体験レビューはループを止めない建付け ②いずれも revert 1コミットで戻せる軽量変更 ③個別に確認時間を取るより実機で自然に目に入る
  - 承認すると: 次回実機で気づいた点だけバックログへ ／ 却下すると: 項目別のレビュー予定を組む ／ 可逆性: あり
  - 回答:
...
- [ ] B-15. 新規アプリ第1弾プロトタイプ「ものがたりダイス」を `drafts/monogatari-dice/` に作成（2026-07-11 提案10承認。drafts/ プロトタイプ制度の条件に全面的に従う。A-8（draft-lint）完了が前提。設計: 「どこで/だれが/なにを」の3ダイスを Material Symbols アイコン＋文字ラベルで表示、ボタンで振る・振り直し・パス自由、入力ゼロ・オフライン・永続化ゼロ。条件（Codex 指摘反映）: お題語彙の組み合わせ全件をコンテンツガイドラインで安全監査した表を同ディレクトリに同梱、アイコンに代替テキスト、prefers-reduced-motion でアニメ停止。完了条件: draft-lint パス＋AIブラウザ動作確認。**apps/ への移植・ポータル掲載は別途 Tier D ゲート**）
...
- [ ] C-5. RTDB 14アプリの掃除カバレッジ監査マトリクス（TTL・終了後削除・onDisconnect。2026-07-11 クロスレビュー両承認で起票、経緯は `docs/reports/cross-review-2026-07-11.md`。条件: 「論理的な期限切れ判定」と「Firebase 上の物理削除」を別列にし、役割（ホスト/ゲスト）×イベント（通常退出/ゲーム終了/再読込/通信断/復帰）で整理。onDisconnect の再登録・取消、サーバー時刻使用、タイマー片付けも対象。kakure-number / uso-jisho / pittari-meter の「退出=ローカル片付けのみ」設計が意図か漏れかを仕分ける。修正候補は Tier 案付きで報告のみ）
- [ ] C-6. `database.rules.json` への `.validate` 追加案の調査（2026-07-11 クロスレビュー両承認で起票。条件: バイトサイズ上限でなく、型・文字列長・子ノード数・必須キー・status 値集合・所有権（他人のデータを書き換えられるか）を対象に。削除時の `.validate` 挙動と全書き込み経路を洗い、Firebase Emulator でのテスト手順案まで落とす。**適用は禁止パスのため Tier D**）
- [ ] C-7. 外部リソース（CDN・フォント等）の棚卸しと SRI 必須 lint（DEP-2）の設計（2026-07-11 クロスレビューで「即 lint 追加」は却下→棚卸し先行に縮小。条件: 全外部リソースを列挙し、SRI を必須にできるもの（固定版の外部JS）とできないもの（Google Fonts 等の応答変動）を仕分け。誤検知パターン（属性の順序・改行）の fixture 検証と警告期間の設計まで含めて報告。lint 実装は別項目として起票し直す）
- [ ] C-8. モーダル/オーバーレイの分類調査と a11y 実装案（2026-07-11 クロスレビューで「role="dialog" の一括付与」は却下→分類先行に縮小。条件: 各アプリのオーバーレイ（再接続・ホスト切断・出題/判定シート等）を dialog（操作あり）/ status / alert（通知のみ）に分類し、dialog にはフォーカス移動・復元・背景 inert 化まで一体の実装案を付けて報告。フォーカス管理なしの `aria-modal` 単独付与は「偽モーダル」として禁止。実装は分類確定後に 1アプリ1コミットの Tier B 項目として起票し直す）
- [ ] C-9. 匿名認証失敗経路の棚卸し（2026-07-11 クロスレビュー両承認で起票。条件: 全 RTDB アプリの `authReady` の await / catch の有無、失敗時に DB 処理へ進んでしまわないか、未処理 rejection・無限スピナーの有無を確認。子ども向けエラー表示＋再試行手段の統一案を Tier 案付きで報告。apps/shared に触る修正案が出た場合はその旨を明記して個別承認へ）
