...
> **2026-09-25 注記**: 本文中の `{漢字|かんじ}` ふりがな記法は、第1期スライドエンジン（apps/kotoba-asobo、2026-08-07 公開終了）だけが `<ruby>` に変換していた。現行アプリは変換しないので、記法がそのまま画面に出る。今の方針は「ふりがなは付けない」（ルート AGENTS.md「表記トーン」）で、apps/ に記法が残ると lint [CONTENT-2] がエラーにする。本文は当時の記録として残す。

あなたはroom-K「コトバであそぼ」のコンテンツ作成担当。指定されたセッション（1回分×N）のデータファイルを作成する。
