...
- 改訂記録:
  - 2026-08-10 実機FBにより「技」という呼び方を「**テーマ**」へ変更し、アプリを「**コトバのテーマクイズ**」（`kotoba-theme`）に改名（旧 `kotoba-waza` は転送スタブ）。あわせて各テーマ8問（計136問）へ増補し毎回4問を抽選、TOPにゲームからの逆引きを追加、橋渡し先は「遊び」のゲームのみに限定
...
| 時間 | パート | 使うもの | 内容 |
|---|---|---|---|
| 0〜5分 | チェックイン | Metalife チャット（必要なら既存ツール。※当初例に挙げた kimochi-map は 2026-09-25 にルーム型の「気持ち当てゲーム」へ作り変え済み） | あいさつ。「パスOK・見てるだけOK」を毎回宣言 |
| 5〜10分 | きょうのコトバのテーマ | **コトバのテーマクイズ**（kotoba-theme。2026-08-10 追加・同日「技」から「テーマ」へ改称） | 今日のテーマを選んで3択クイズを2〜4問（画面共有・チャットで①②③回答）。まとめ画面の「きょうのゲームでつかってみよう」からそのままゲームへ。講義をしない |
...
- **「コトバのテーマの紹介」= コトバのテーマクイズ（2026-08-10 改訂）**。17テーマ×各8問から毎回4問を抽選する3択ミニクイズでテーマを先に体験し、まとめ画面が当日のゲーム・パックへ橋渡しする。全問やる必要はなく2問で切り上げてよい。成績・正解数は表示しない（テストにしない）。45分全体の進行管理は引き続きアプリ化しない（第2.2章の原則は不変。テーマクイズは quiz と同じ「1スロットを担うコンテンツ側のツール」）
...
- 品質基準は第1期 [05-data-schema.md](./kotoba-asobo/05-data-schema.md) 第4節を継承する（誤答にも選ばれる理由がある・断定しすぎない・パスを認める文面・個人情報を答えさせない 等）。ただし「難しい漢字は `{|}` 記法でふりがな」の項目は継承しない（表記はルート AGENTS.md「表記トーン」に従い、ふりがなは付けない）
...
| 間違い文章探偵 | 既存 machigai-sagashi で実現していたが 2026-08-10 に一旦削除（git 履歴に残存）。再挑戦時は復元または新規設計 |
