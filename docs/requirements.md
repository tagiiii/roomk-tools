...
- 作成日: 2026-08-09
- ステータス: **承認済み・実装中**（2026-08-09 オーナー承認「おまかせします」。未決事項1〜9は第34章の推奨案どおり確定。本書が「コトバであそぼ！第2期（ゲーム版）」の正本）
...
| 5〜10分 | きょうのコトバの技 | 口頭＋ゲームの「あそびかた」モーダル画面共有 | 今日のパック名（例:「はんたいことば」）を紹介。講義をしない。1〜2分で技の例を1つ見せ、すぐゲームへ |
...
- **「コトバの技の紹介」= パック選択**。パック名とその1例を見せることが技の紹介そのものになる。専用の紹介スライド機能は作らない（Could: 各アプリの slides.html にルール説明スライドを追加する既存文化には乗る）
...
| モード | ルール | ねらい |
|---|---|---|
| **オープン**（MVP必須・既定） | 全カード表向き。手番の人が「つながる2枚」を選んで宣言する | 記憶力不要。ことばの意味だけに集中でき、相談が生まれる。Lv.1 の子も見て考えられる |
| **クローズ**（Should） | 全カード裏向き。手番の人が2枚めくる。つながればそのまま、違えば裏に戻る | 記憶ゲームの緊張感。めくった札を全員が覚えているので自然と協力が生まれる |
...
| MVPで必要な範囲 | オープンモード・パック3本・協力のみ・ヒントは「そろえて見せる」まで |
| 将来的な拡張 | クローズモード（Should）／おじゃまカード（どれともつながらない1枚）／個人戦オプション／タイムアタックではなく「手数記録」の继続（同日内のみ） |
...
- ホストの「コードをコピー」ボタンで、入室案内（URL＋コード）を Metalife チャットに貼れる形式で出力する
...
- 表記トーン: 子どもがひとりで読む前提の「ことば系」アプリのため、**ひらがな中心のやわらかい表記**（AGENTS.md 2026-08-06 方針の区分どおり）
