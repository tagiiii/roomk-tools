...
- ~~**B2 の follow-up（Codex 指摘・重要）**: pittari-meter に `onDisconnect().cancel()` がどこにも無く、`roomRef.off()` は予約済み `onDisconnect()` を解除しない → 後日の切断で削除済み room を部分再生成しうる。family 横断で同じ穴の可能性大（要棚卸し）。~~ → **2026-07-15 棚卸し＋緩和実装（B-21）**。実際は **family「6」ではなく host/guest 型 RTDB 全13本**で `roomRef.onDisconnect().update` を張り `.cancel()` は皆無と確定。共有ヘルパ `RoomkRTDB.cancelRoomOnDisconnect` を追加し全13本の teardown で off/remove の前に cancel（remove 経路は await）。ブランチ `fix-ondisconnect-cancel-ghost-rooms`・**PR #7 マージ済（2026-07-15・merge `8cadb41`・⚠️実機 E2E 未実施のままオーナー判断マージ＝要 runtime 確認・問題時 revert）**。二重レビュー（独立=承認／Codex=teardown 経路は妥当）＋stub 9/9 pass＋lint 緑。オーナー判断で **Option 1（teardown 緩和＋残り follow-up）** に確定。詳細と未カバー follow-up（DONE タイマー経路・strict cancel・再入ガード・jinro listener detach・**validation rules バックストップ=構造的完全修正だが要エミュレータ**）は [kaizen-backlog.md](../kaizen-backlog.md) B-21 参照。
