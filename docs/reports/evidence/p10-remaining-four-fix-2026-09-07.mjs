// Fixed approved ru18/zk201/zk203/zk211 snapshot. Read-only; no network or OS clipboard.
// Historical validators retain their old SHA requirements; this does not close P-10/C3.
...
const sourceCommit = 'e2cd625f28f0d3dc1c600660d0b23d6b37ae3847';
const beforeSha256 = '79c752e1ae24074477f414196188a006322b573a5bb956dfc850dc2f0a32c194';
const afterSha256 = '65205894c67f6fea1a2a76c908bc78344ce944194fcfe257a56630ed8dc3ff15';
const proposalPath = 'docs/reports/p10-remaining-four-proposal-2026-09-07.md';
const proposalSha256 = '586c97f78085de9ddcf7fb20c32cd11f73ef8fce080436331f0845df2c9aa6e9';
const allowedFields = {
  ru18: ['question', 'choices', 'explanation'],
  zk201: ['question', 'explanation'],
  zk203: ['question', 'choices', 'explanation'],
  zk211: ['question', 'choices', 'explanation'],
};
const ids = Object.keys(allowedFields);
const answerIndices = { ru18: 1, zk201: 0, zk203: 0, zk211: 0 };
...
for (let p = 0; p < before.length; p++) {
  ...
  for (let i = 0; i < bq.length; i++) {
    ...
    if (Object.hasOwn(allowedFields, a.id)) {
      ...
      assert.equal(a.answerIndex, answerIndices[a.id]);
...
// Exactly eleven single-line literal replacements also protect comments/spacing.
...
const priorRetired = ['ky01','ky04','ky18','ky20','sk03','sk05','zk204','z07','s06','s08','z08','ks08','ks13','kj19','kbn07'];
const beforeRemaining = rows.filter(r => r.outcome === 'unresolved' && !priorRetired.includes(r.id)).map(r => r.id).sort();
assert.deepEqual(beforeRemaining, ['ru18','zk201','zk203','zk211']);
for (const id of beforeRemaining) {
  assert.deepEqual(rows.find(r => r.id === id).currentQuestion, questionFields(previous.get(id)));
}
const remainingIDs = beforeRemaining.filter(id => !ids.includes(id));
assert.deepEqual(remainingIDs, []);

// Actual unchanged formatter/copy functions; only clipboard transfer is stubbed.
...
console.log(JSON.stringify({
  ...
  retirementMeaning: 'Four tracked additional-verification items addressed by approved replacement wording; not new proof of the old claims, and not P-10/C3 operational closure.',
  clipboardTextCheck: 'PASS: eight texts via actual copy functions with sink stub; no OS clipboard transfer',
