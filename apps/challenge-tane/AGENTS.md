...
- たねの status は4種類で**全部同格**。「試す」だけでなく「ながめる・保留・もういいかな」も正式な扱いで、成功/失敗の順序をつけない。
- 「たね」という比喩は screen-start の一文（「やってみたいかも」と思ったことを、ここでは「たね」と呼ぶよ）で**一度だけ説明**する。「まく」などの園芸系の派生語は使わず、「増やす」「なおす」「消す」の素直な動詞を使う（比喩の密度を上げない）。
- UI 文言は**通常表記（漢字まじり）**（2026-08-06 ユーザー判断で分かち書きから刷新。ルート AGENTS.md「表記トーン」参照。読めない子にはメンターが読み上げる。「たね」「なおす」などかな書きが自然な語はそのまま）。
...
- `seed.status`: `try（試してみたい） | watch（ながめておく） | hold（保留） | close（もういいかな）`。4つは同格の enum で遷移制限なし。
