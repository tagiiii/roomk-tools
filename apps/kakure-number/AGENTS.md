...
- `thinking → reveal` は「配布済みかつ在室中のゲスト全員が提出」でホスト端末が自動実行。未提出者がいても、予想が1件以上あればホストが「締め切る」で強制遷移できる。
...
- 予想が出ない子がいても「締め切る」で先に進める（未提出は「よそうなし」表示になるだけで、責める演出はない）。
