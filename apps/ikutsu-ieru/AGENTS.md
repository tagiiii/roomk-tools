...
- **期待**: ゲストに `host-off-overlay` が即時表示。復帰するとオーバーレイが消えて元の画面に戻る。2 分(`ORPHAN_TTL_MS`)を超えて切断が続くとルームが自動削除されゲストはトップへ戻る
...
- **期待**: 30 秒後にホスト端末の削除 transaction が確定し、全端末がトップへ戻る。対象の `ikutsu_rooms/{code}` が消えている
