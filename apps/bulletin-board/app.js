...
async function handleLogin(e) {
  ...
  if (!/^\d{4}$/.test(pin)) { showError("login-error", "パスワードは4桁の数字で入力してください"); return; }
  ...
  try {
    ...
    if (snap.data().pinHash !== pinH) { showError("login-error", "パスワードが正しくありません"); return; }
...
async function handleRegister(e) {
  ...
  if (!/^\d{4}$/.test(pin)) { showError("register-error", "パスワードは4桁の数字で入力してください"); return; }
  if (pin !== pinConfirm) { showError("register-error", "パスワードが一致しません"); return; }
