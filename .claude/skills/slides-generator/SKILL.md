...
**正本テンプレート: `apps/codenames/slides.html`**（type 分岐のあるデータ駆動版）。
hint-de-pinto 版はフラット構造の旧型なので、新規作成時は必ず codenames 版をベースにすること。実装が分岐すると保守できなくなるため、独自エンジンを書き起こさない。

## 作成手順

1. `apps/codenames/slides.html` を読み、構造を把握する:
...
2. エンジン部分（ナビゲーション・同期・fit・コピー）は**そのまま流用**し、BroadcastChannel のチャンネル名と localStorage キーの接頭辞だけアプリ固有に変える
3. カスタマイズしてよいのは: `SLIDES` の中身、`:root` の配色トークン、フォント
4. スライド構成の目安（codenames は 16枚）:
...
- 発表者ノート（`note`）は口語体で、そのまま読み上げ・Metalife チャット貼り付けできる文章にする
- 番号付き選択肢を見せる場合は丸数字 `①②③`
...
- `bash scripts/lint.sh` がエラー0のまま通ること
